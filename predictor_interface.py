...
class CollegePredictor:
    ...
    def get_recommendations_by_rank(self, rank: int, category: str = 'OPEN', top_n: int = 20):
        ...
        filtered_data = filtered_data.dropna(subset=['closing_rank'])
        filtered_data = filtered_data[filtered_data['closing_rank'] > 0]
        
        # Group by institute and branch
        ...
        # 1. Find suitable colleges
        reach_threshold = rank * 0.8 
        suitable = grouped[grouped['closing_rank'] >= reach_threshold].copy()

        if len(suitable) == 0:
            return pd.DataFrame()

        # --- START: NEW LOGARITHMIC PROBABILITY ---

        # 2. Calculate a new, log-based probability
        def calculate_probability(row):
            closing_rank = row['closing_rank']
            
            # Use a ratio for log scale.
            # We use max(1, rank) to avoid division by zero if rank is 0.
            # We use max(0.1, ...) to avoid log(0) or log(negative)
            ratio = closing_rank / max(1, rank)
            ratio = max(0.1, ratio) # Prevent errors on reach colleges
            
            # log10(1) = 0 (when rank == closing_rank -> 50%)
            # log10(10) = 1 (when rank 1k, closing 10k -> 90%)
            # log10(0.5) = -0.3 (when rank 10k, closing 5k -> 38%)
            
            # We use a factor (e.g., 40) to control the "steepness"
            probability = 50 + (40 * math.log10(ratio))
            
            # Clamp the probability between 5% and 95%
            return max(5.0, min(95.0, probability))

        suitable['probability'] = suitable.apply(calculate_probability, axis=1).round(1)
        
        # --- END: NEW LOGARITHMIC PROBABILITY ---
        
        # Sort by probability (highest first) and then by closing rank
